...
void * Wifibot::write_th(void *arg) {
	...
	while (1) {
		...
		//printf("write_th has run. Checkpoint 21. \n\r");
		//printf("l = %u \n\r", l);
		//printf("r = %u \n\r", r);
		...
		//printf("write_th has finished. Checkpoint 22. \n\r");
		usleep(10000);
...
void Wifibot::cmd_vel_callback(const geometry_msgs::TwistConstPtr msg) {
	
	double vel_x = msg->linear.x;
	double vel_th = msg->angular.z;
	double width = 0.33;
	signed int speed_l_intermediate;
	signed int speed_r_intermediate;

	bool dir_l, dir_r;
	
	//ROS_INFO(msg->linear.x);
	
	//printf("CMD_VEL Callback has run. Checkpoint 1. \n\r");
	//printf("vel_x: %f \n",vel_x); 
	//printf("vel_th: %f \n",vel_th);        

	//ROS_INFO("CMD_VEL Callback has run. Checkpoint 1.");

	// Old way of doing reverse - doesn't work correctly with rotation. 
	/*
	if(vel_x < 0){
		vel_x = -vel_x;
		dir_l = true;
	...
	*/
	...
	speed_l_intermediate = (signed int)(200000 * ( vel_x - vel_th * width / 2.0));
	speed_r_intermediate = (signed int)(200000 * ( vel_x + vel_th * width / 2.0));

	// New way of doing reverse.
	if(speed_l_intermediate < 0){
		printf("Getting opposite for speed_l_intermediate. \n\r");
		speed_l_intermediate = abs(speed_l_intermediate);
		dir_l = true;
	}else{
		dir_l = false;
	}
	if(speed_r_intermediate < 0){
		printf("Getting opposite for speed_r_intermediate. \n\r");
		speed_r_intermediate = abs(speed_r_intermediate);
		dir_r = true;
	}else{
		dir_r = false;
	}

	printf("CMD_VEL Callback has run. Checkpoint 2. \n\r");
	printf("speed_l: %d \n",speed_l_intermediate); 
	printf("speed_r: %d \n",speed_r_intermediate);  
	printf("\n");      
		


	this->speed_cmd_l = speed_l_intermediate;
;
	this->speed_cmd_r = speed_r_intermediate;
